    ...
    async throws -> [[String: Any]]
    {
        ...
        guard response.statusCode == 200 else {
            ...
            do {
                ...
                if let error = try? decoder.decode(VaporErrorResponse.self, from: data) {
    ...
    async throws -> AsyncThrowingStream<T, Error>
    {
        ...
        guard response.statusCode == 200 else {
            ...
            do {
                ...
                if let error = try? decoder.decode(OpenAIErrorResponse.self, from: errorData) {
                    errorMessage = error.error.message ?? "NO ERROR MESSAGE PROVIDED"
                }
                
                if let error = try? decoder.decode(VaporErrorResponse.self, from: errorData) {
                    errorMessage = error.reason
                }
